...
/* David: ¿Es realmente necesaria está función? ¿Se va a utilizar fuera de este paquete?
 * ¿Por qué no utilizar directamente size-1?
 */
...
void * stack_pop(Stack *s) {
	...
	obj = s->items[top(s)];
	s->size--;
	/*David: Considerar si habría que poner la posición top a NULL -> s->items[top(s)] = NULL */
	aux = (void**) realloc(s->items, s->size * sizeof (void*));
...
int stack_print(FILE* fp, const Stack* s) {
	...
	if (fp == NULL || s == NULL || s->items == NULL || s->size <= 0)
		return 0;
	/* David: ¿Cuál es el propósito de printedChars? A la hora de imprimir, es print_element_function quien se encarga por lo que
	 * s->print_element_function(fp, s->items[i]) sería suficiente. Esta función devolvería siempre 0
	 */
